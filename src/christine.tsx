...
const TRELLO_LIST_ID = preferences.listIdChristine;
const TRELLO_MEMBERS = `${preferences.userIdChristine},${preferences.userIdAnu}`;
...
export default function Command() {
  ...
  async function handleSubmit(values: Values) {
    ...
    try {
      ...
      if (response.status === 200 || response.status === 201) {
        ...
        savedLinks.push({ url, title: values.cardTitle, command: "Christine" });
  ...
  return (
    ...
    >
      <Form.Description text="Let's create a Trello Card for Christine in the Sauers Board." />
